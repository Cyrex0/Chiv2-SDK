#pragma once

/**
 * Name: chiv2
 * Version: 123456
 */

#ifdef _MSC_VER
	#pragma pack(push, 0x01)
#endif

namespace CG
{
	// --------------------------------------------------
	// # Structs
	// --------------------------------------------------
	/**
	 * Function WeaponPoleHammerProgression.WeaponPoleHammerProgression_C.OnAssemble_WeaponPoleHammerProgression
	 */
	struct UWeaponPoleHammerProgression_C_OnAssemble_WeaponPoleHammerProgression_Params
	{
	};

	/**
	 * Function WeaponPoleHammerProgression.WeaponPoleHammerProgression_C.ExecuteUbergraph_WeaponPoleHammerProgression
	 */
	struct UWeaponPoleHammerProgression_C_ExecuteUbergraph_WeaponPoleHammerProgression_Params
	{
	public:
		int                                                        EntryPoint;                                              // 0x0000(0x0004)  (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)

	};

}

#ifdef _MSC_VER
	#pragma pack(pop)
#endif
