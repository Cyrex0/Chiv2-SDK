#pragma once

/**
 * Name: chiv2
 * Version: 123456
 */

#ifdef _MSC_VER
	#pragma pack(push, 0x01)
#endif

namespace CG
{
	// --------------------------------------------------
	// # Structs
	// --------------------------------------------------
	/**
	 * Function WeaponRapierProgression.WeaponRapierProgression_C.OnAssemble_WeaponRapierProgression
	 */
	struct UWeaponRapierProgression_C_OnAssemble_WeaponRapierProgression_Params
	{
	};

	/**
	 * Function WeaponRapierProgression.WeaponRapierProgression_C.ExecuteUbergraph_WeaponRapierProgression
	 */
	struct UWeaponRapierProgression_C_ExecuteUbergraph_WeaponRapierProgression_Params
	{
	public:
		int                                                        EntryPoint;                                              // 0x0000(0x0004)  (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)

	};

}

#ifdef _MSC_VER
	#pragma pack(pop)
#endif
