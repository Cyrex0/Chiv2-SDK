#pragma once

/**
 * Name: chiv2
 * Version: 123456
 */

#ifdef _MSC_VER
	#pragma pack(push, 0x01)
#endif

namespace CG
{
	// --------------------------------------------------
	// # Structs
	// --------------------------------------------------
	/**
	 * Function WeaponFalchionProgression.WeaponFalchionProgression_C.OnAssemble_WeaponFalchionProgression
	 */
	struct UWeaponFalchionProgression_C_OnAssemble_WeaponFalchionProgression_Params
	{
	};

	/**
	 * Function WeaponFalchionProgression.WeaponFalchionProgression_C.ExecuteUbergraph_WeaponFalchionProgression
	 */
	struct UWeaponFalchionProgression_C_ExecuteUbergraph_WeaponFalchionProgression_Params
	{
	public:
		int                                                        EntryPoint;                                              // 0x0000(0x0004)  (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)

	};

}

#ifdef _MSC_VER
	#pragma pack(pop)
#endif
