#pragma once

/**
 * Name: chiv2
 * Version: 123456
 */

#ifdef _MSC_VER
	#pragma pack(push, 0x01)
#endif

namespace CG
{
	// --------------------------------------------------
	// # Structs
	// --------------------------------------------------
	/**
	 * Function WeaponThrowingMalletProgression.WeaponThrowingMalletProgression_C.OnAssemble_WeaponThrowingMalletProgression
	 */
	struct UWeaponThrowingMalletProgression_C_OnAssemble_WeaponThrowingMalletProgression_Params
	{
	};

	/**
	 * Function WeaponThrowingMalletProgression.WeaponThrowingMalletProgression_C.ExecuteUbergraph_WeaponThrowingMalletProgression
	 */
	struct UWeaponThrowingMalletProgression_C_ExecuteUbergraph_WeaponThrowingMalletProgression_Params
	{
	public:
		int                                                        EntryPoint;                                              // 0x0000(0x0004)  (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)

	};

}

#ifdef _MSC_VER
	#pragma pack(pop)
#endif
