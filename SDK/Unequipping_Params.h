#pragma once

/**
 * Name: chiv2
 * Version: 123456
 */

#ifdef _MSC_VER
	#pragma pack(push, 0x01)
#endif

namespace CG
{
	// --------------------------------------------------
	// # Structs
	// --------------------------------------------------
	/**
	 * Function Unequipping.Unequipping_C.OnAssemble_Unequipping
	 */
	struct UUnequipping_C_OnAssemble_Unequipping_Params
	{
	};

	/**
	 * Function Unequipping.Unequipping_C.EventBeginState
	 */
	struct UUnequipping_C_EventBeginState_Params
	{
	public:
		class FName                                                PreviousState;                                           // 0x0000(0x0008)  (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)
		float                                                      PreviousStateTime;                                       // 0x0008(0x0004)  (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)
		float                                                      AnimationLength;                                         // 0x000C(0x0004)  (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)

	};

	/**
	 * Function Unequipping.Unequipping_C.EventEndAnimation
	 */
	struct UUnequipping_C_EventEndAnimation_Params
	{
	};

	/**
	 * Function Unequipping.Unequipping_C.Timeout
	 */
	struct UUnequipping_C_Timeout_Params
	{
	};

	/**
	 * Function Unequipping.Unequipping_C.ExecuteUbergraph_Unequipping
	 */
	struct UUnequipping_C_ExecuteUbergraph_Unequipping_Params
	{
	public:
		int                                                        EntryPoint;                                              // 0x0000(0x0004)  (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)

	};

}

#ifdef _MSC_VER
	#pragma pack(pop)
#endif
