#pragma once

/**
 * Name: chiv2
 * Version: 123456
 */

#ifdef _MSC_VER
	#pragma pack(push, 0x01)
#endif

namespace CG
{
	// --------------------------------------------------
	// # Structs
	// --------------------------------------------------
	/**
	 * Function WeaponSledgeHammerProgression.WeaponSledgeHammerProgression_C.OnAssemble_WeaponSledgeHammerProgression
	 */
	struct UWeaponSledgeHammerProgression_C_OnAssemble_WeaponSledgeHammerProgression_Params
	{
	};

	/**
	 * Function WeaponSledgeHammerProgression.WeaponSledgeHammerProgression_C.ExecuteUbergraph_WeaponSledgeHammerProgression
	 */
	struct UWeaponSledgeHammerProgression_C_ExecuteUbergraph_WeaponSledgeHammerProgression_Params
	{
	public:
		int                                                        EntryPoint;                                              // 0x0000(0x0004)  (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)

	};

}

#ifdef _MSC_VER
	#pragma pack(pop)
#endif
